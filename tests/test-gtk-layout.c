/*
  test-gtk-layout.c -- Measure fullscreen write speed under GTK
  ...
  gcc -Wall -O2 ` pkg-config --libs --cflags gtk+-2.0` test-gtk-layout.c -o test-gtk-layout
...
x_blit(void)
{
  ...
  if (Verbose)
    {
      printf("test-gtk-layout: Font loaded, drawable + color created in %lli ms\n",
  ...
  for (j=0; j<TotalCycles; j++)
      ...
      for (i=0; i<TextNLines; i++)
	{
          ...
          gdk_draw_layout (window->window, window->style->black_gc, x, y, layout);
  ...
  printf("test-gtk-layout: Total time %lli ms, %i glyphs rendered = approx %lli glyphs per second\n",
...
{
  fprintf(stderr, 
	  "test-gtk-layout " VERSION "\n"
	  "usage: test-gtk-layout [options..]\n"
          "Options are;\n"
          "-display <X display>\n"
	  "--verbose\n"
